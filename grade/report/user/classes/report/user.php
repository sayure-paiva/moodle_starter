...
class user extends grade_report {
    ...
    private function fill_table_recursive(array &$element) {
        ...
        if ($type == 'category') {
            ...
            // Alter style based on whether aggregation is first or last.
            if ($this->switch) {
                $data['itemname']['class'] = $class . ' ' . "d$depth b1b b1t category". " table-bg";
            } else {
    ...
    public function print_table(bool $return = false) {
        ...
        $table->attributes = [
            ...
            'class' => 'generaltable boxaligncenter user-grade table-bg text-secondary-light',
        ];
        ...
        foreach ($this->tableheaders as $index => $heading) {
            ...
            $headingcell->attributes['class'] = "header table-bg text-secondary column-{$this->tablecolumns[$index]} ";
            if ($index == 0) {
        ...
        foreach ($this->tabledata as $rowdata) {
            ...
            if (isset($rowdata['leader'])) {
                ...
                $rowspancell->attributes['class'] = $rowdata['leader']['class'].' table-bg';
                $rowspancell-> attributes['class']= "table-bg text-secondary";
            ...
            foreach ($this->tablecolumns as $tablecolumn) {
                ...
                if (!is_null($content)) {
                    ...
                    if (isset($rowdata[$tablecolumn]['colspan'])) {
                        $rowcell->colspan = $rowdata[$tablecolumn]['colspan table-bg'];
                    }
            ...
            $tablerow->attributes = ['class' => $classes.' table-bg', 'data-hidden' => 'false'];
        ...
        if ($return) {
            return \html_writer::div($userreporttable, 'user-report-container', ['id' => "user-report-{$this->user->id}",'class'=>'rounded'],);
        }
