...
class core_question_renderer extends plugin_renderer_base {
    ...
    public function question(question_attempt $qa, qbehaviour_renderer $behaviouroutput,
        ...
        $output = '';
        $quiz_estudante = is_siteadmin($USER) ? '' : 'quiz-soulcode-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::start_tag('div', array(
            'id' => $qa->get_outer_question_div_unique_id(),
            'class' => implode(' ', array(
                'que', ' '.$quiz_estudante.' ', //aqui chamo o resultado do ternário
        ...
        $perguntas_estudante = is_siteadmin($USER) ? '' : 'quiz-perguntas-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::tag('div',
                $this->info($qa, $behaviouroutput, $qtoutput, $options, $number),
                array('class' => 'info rounded ' .$perguntas_estudante. ' ')); //aqui chamo o resultado do ternário

        $output .= html_writer::start_tag('div', array('class' => 'content'));


        $respostas_estudante = is_siteadmin($USER) ? '' : 'quiz-respostas-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::tag('div',
                $this->add_part_heading($qtoutput->formulation_heading(),
                $this->formulation($qa, $behaviouroutput, $qtoutput, $options)),
                array('class' => 'formulation clearfix ' .$respostas_estudante. '')); //aqui chamo o resultado do ternário
        
        $combo_success_alert_estudante = is_siteadmin($USER) ? '' : 'combo-success-alert-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::nonempty_tag('div',
                $this->add_part_heading(get_string('feedback', 'question'),
                    $this->outcome($qa, $behaviouroutput, $qtoutput, $options)),
                array('class' => 'outcome clearfix combo-success-alert '.$combo_success_alert_estudante.'')); //aqui chamo o resultado do ternário
        $output .= html_writer::nonempty_tag('div',
