...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output = '';
        $output .="<div class='review-table'>";
        $quizreviewsummary_estudante = is_siteadmin($USER) ? '' : 'quizreviewsummary_estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox quizreviewsummary '.$quizreviewsummary_estudante.''));
    ...
    public function finish_review_link(quiz_attempt $attemptobj) {
        ...
        $mod_quiz_next_nav_estudante = is_siteadmin($USER) ? '' : 'mod_quiz-next-nav-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            ...
            return html_writer::empty_tag('input', array('type' => 'button',
                    ...
                    'class' => 'mod_quiz-next-nav btn btn-primary '.$mod_quiz_next_nav_estudante.''));

        } else {
            return html_writer::link($url, get_string('finishreview', 'quiz'),
                    array('class' => 'mod_quiz-next-nav btn btn-primary-light '.$mod_quiz_next_nav_estudante.''));
        }
    }
    ...
    public function navigation_panel(quiz_nav_panel_base $panel) {
        ...
        $qn_buttons_estudante = is_siteadmin($USER) ? '' : 'qn_buttons_estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $output .= html_writer::start_tag('div', array('class' => "qn_buttons clearfix $bcc $qn_buttons_estudante")); //aqui chamo o resultado do ternário
    ...
    public function during_attempt_tertiary_nav($quizviewurl): string {
        ...
        $btn = is_siteadmin($USER) ? '' : 'mod_quiz-back-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
    ...
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        $output .= html_writer::start_tag('div', array('class' => 'submitbtns'));
        $btn = is_siteadmin($USER) ? '' : 'mod_quiz-prev-nav-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'quiz'), 'class' => 'mod_quiz-prev-nav btn btn-secondary '.$btn.'',
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_quiz-next-nav teste btn btn-primary-light '.$btn.'', 'id' => 'mod_quiz-next-nav'));
        $output .= html_writer::end_tag('div');
    ...
    public function summary_page($attemptobj, $displayoptions) {
        ...
        $output .= $this->header();
        $output .= $this->during_attempt_tertiary_nav($attemptobj->view_url());
        $output .= '<div class="d-flex flex-column justify-content-center align-items-center attemps-table">';
        $output .= $this->heading(format_string($attemptobj->get_quiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'quiz'), 3);
        $output .= '</div>';
        
        $output .= '<div class="d-flex justify-content-center attemps-table">';
    ...
    public function summary_table($attemptobj, $displayoptions) {
        global $USER;
        $quizsummaryofattempt_estudante = is_siteadmin($USER) ? '' : 'quizsummaryofattempt_estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.

        // Prepare the summary table header.        
        $table = new html_table();
        $table->attributes['class'] = 'generaltable quizsummaryofattempt '.$quizsummaryofattempt_estudante.' boxaligncenter m-auto';
    ...
    public function summary_page_controls($attemptobj) {
        ...
        $quiz_btn_attempt_review = is_siteadmin($USER) ? '' : 'quiz_btn_attempt_review'; //verifica se o usuário é estudante para alterar a classe abaixo.
        // Return to place button.
        if ($attemptobj->get_state() == quiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'quiz'));
            $button->class .=' '.$quiz_btn_attempt_review.'';
        ...
        $quiz_btn_attempt_finish = is_siteadmin($USER) ? 'btn-finishattempt' : 'btn-finishattempt quiz_btn_attempt_finish'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $button->class = $quiz_btn_attempt_finish;
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        ...
        $output .= "<div class='resumo-notas-table'>";
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
        ...
        // Verifica se o usuário é admin
            if (has_capability('mod/folder:managefiles', context_system::instance())) {
                $button->class .= ' quizstartbuttondiv';
            } else {
                $button->class .= ' quizstartbuttondiv quiz-estudante';
            }
    ...
    public function view_table($quiz, $context, $viewobj) {
        ...
        $table = new html_table();
        $quizattemptsummary_estudante = is_siteadmin($USER) ? '' : 'quizattemptsummary-estudante'; //verifica se o usuário é estudante para alterar a classe abaixo.
        $table->attributes['class'] = 'generaltable quizattemptsummary rounded '.$quizattemptsummary_estudante.''; // aqui chamo a variavel
        $table->head = array();
        $table->align = array();
        $table->size = array();
